...
class BspGroupPooling(GroupPooling):
    ...
    def fourier_transform_vectorized_batch_dihedral(self, f):
        ...
        if n % 2 == 0:
            ...
            fhat[:, :, 1, 1, 0] = f[:, :, 0:n:2].sum(axis= 2) - f[:, :, 1:n:2].sum(axis=2) - f[:, :, n:2*n:2].sum(axis= 2) + f[:, :, n+1:2*n:2].sum(axis = 2)
    ...
    def bispectrum_full_vectorized_batch_cyclic(self, x):
        ...
        for j in range(n):
            temp = fhat[..., irange[j:]] * fhat[..., j].unsqueeze(2) * torch.conj(fhat[..., (irange[j:] + j) % n])
            endreal = count + n - j
    ...
    def bispectrum_vectorized_batch_dihedral(self, x, n):
        ...
        CBmatrix, indices = self.CBmatrices[0, ...].to(device), self.indices[:, 0].to(device) #first_last_cb(n, end=False)
        ...
        for i in range(2, n2):
            CBmatrix, indices = self.CBmatrices[i - 1, ...].to(device), self.indices[:, i - 1].to(device)
        ...
        if n % 2 == 0:
            CBmatrix, indices = self.CBmatrices[n2 - 1, ...].to(device), self.indices[:, n2 - 1].to(device)#first_last_cb(n, end=True)
