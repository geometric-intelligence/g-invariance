...
class TCGroupPooling(GroupPooling):
    ...
    def triple_correlation_vectorized_batch_cyclic(self, x):
        ...
        # print("rolls_mult__shape",rolls_mult.shape)
        # print("all_rolls_shape",all_rolls.shape)
...
class AvgGroupPooling(EquivariantModule):
    def __init__(self, in_type: FieldType, **kwargs):
        ...
        acts as identity).
        For each input field, an output field is built by taking the mean of the activation
        within that field; as a result, the output field transforms according to a trivial representation.
        ...
        Args:
            ...
            pooling_type: type of pooling (max, avg)
        ...
        super(AvgGroupPooling, self).__init__()
    ...
    def forward(self, input: GeometricTensor) -> GeometricTensor:
        ...
        output = torch.empty(
            self.evaluate_output_shape(input.shape), device=input.device, dtype=torch.float
        )
        ...
        for s, contiguous in self._contiguous.items():
            ...
            activations = torch.mean(fm, dim=2)

            if contiguous:
                output[:, out_indices[0] : out_indices[1], ...] = activations
            else:
                output[:, out_indices, ...] = activations
