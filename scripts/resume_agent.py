...
from transform_datasets.utils.wandb import load_dataset
...
parser.add_argument("--run_id", type=str, help="wandb run_id.")
parser.add_argument("--entity", default="", type=str, help="wandb entity name.")
...
parser.add_argument(
    "--n_examples",
    type=int,
    help="number of data examples to resume up to.",
    default=int(1e7),
)
...
parser.add_argument(
    "--dataset_config",
    type=str,
    help="Optional dataset config if you want to resume on a different dataset.",
    default=None,
)
...
wandb_run = wandb.init(
    project=args.project, entity=args.entity, id=args.run_id, resume="must"
)
...
dataset = load_dataset(
    dataset_config, project=trainer.logger.data_project, entity=args.entity
)
...
if trainer.epoch >= epochs:
    print(
        "Model has already been trained for {} examples. Skipping to next run.".format(
            args.n_examples, trainer.n_examples
        )
    )
else:
    remaining_epochs = epochs - trainer.epoch

    trainer.resume(data_loader=data_loader, epochs=remaining_epochs)
